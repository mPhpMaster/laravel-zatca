<?php

namespace MPhpMaster\ZATCA\Test\Unit;

use MPhpMaster\ZATCA\TagBag;
use MPhpMaster\ZATCA\Tags\Company;
use MPhpMaster\ZATCA\Tags\InvoiceDate;
use MPhpMaster\ZATCA\Tags\InvoiceTotalAmount;
use MPhpMaster\ZATCA\Tags\VatAmount;
use MPhpMaster\ZATCA\Tags\VatId;
use MPhpMaster\ZATCA\Test\TestCase;

class TagBagTest extends TestCase
{
    /** @test */
    public function shouldGenerateAQrCode()
    {
        $value = TagBag::make()
            ->addCompany('Company name')
            ->addVatId('1234567891')
            ->addInvoiceDate('2021-11-24T03:48:00Z')
            ->addInvoiceTotalAmount('100')
            ->addVatAmount('15')
            ->toBase64();

        $this->assertEquals('AQxDb21wYW55IG5hbWUCCjEyMzQ1Njc4OTEDFDIwMjEtMTEtMjRUMDM6NDg6MDBaBAMxMDAFAjE1', $value);
    }

    /** @test */
    public function shouldGenerateAQrCodeAsArabic()
    {
        $value = TagBag::make()
            ->addCompany('المنشآة')
            ->addVatId('1234567891')
            ->addInvoiceDate('2021-11-24T03:48:00Z')
            ->addInvoiceTotalAmount('100')
            ->addVatAmount('15')
            ->toTLV();

        $this->assertEquals('AQ7Yp9mE2YXZhti02KLYqQIKMTIzNDU2Nzg5MQMUMjAyMS0xMS0yNFQwMzo0ODowMFoEAzEwMAUCMTU=', base64_encode($value));
    }

    /** @test */
    public function shouldGenerateAQrCodeFromTagsClasses()
    {
        $generatedString = TagBag::make()
            ->addTag(Company::TAG, 'Company name')
            ->addTag(VatId::TAG, '1234567891')
            ->addTag(InvoiceDate::TAG, '2021-11-24T03:48:00Z')
            ->addTag(InvoiceTotalAmount::TAG, '100')
            ->addTag(VatAmount::TAG, '15')
            ->toBase64();

        $this->assertEquals('AQxDb21wYW55IG5hbWUCCjEyMzQ1Njc4OTEDFDIwMjEtMTEtMjRUMDM6NDg6MDBaBAMxMDAFAjE1', $generatedString);
    }

    /** @test */
    public function shouldGenerateAQrCodeDisplayAsImageData()
    {
        $value = TagBag::make()
            ->addCompany('المنشآة')
            ->addVatId('1234567891')
            ->addInvoiceDate('2021-11-24T03:48:00Z')
            ->addInvoiceTotalAmount('100')
            ->addVatAmount('15')
            ->toImage();

        $this->assertEquals('data:image/png;base64,', $value);
    }
}
